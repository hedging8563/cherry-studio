...
| orchestrator / `BackupService` / `RestoreSafetyManager` | C / D / E | Export / restore / safety. **Blocked on upstream gating** (DbService `createSnapshot` / `restoreDbFromSnapshot` / `verifyLiveDb`, PreferenceService `reloadFromDb` / `armWriteGate`, lifecycle `@WriteSilenceable` decorators, business-Service restore hooks) — see `/Users/gd32/Downloads/backup-upstream-issues/`. |
