...
| orchestrator / `BackupService`（+ ~~`RestoreSafetyManager`~~ 已废弃） | C / D / ~~E~~ | Export / restore。**D 模型**：runtime detached merge into `work.sqlite` + preboot atomic promotion（`RestoreSafetyManager` / runtime rollback 整组废弃——safety 由 preboot promotion gate + 重启结构性保证，见 backup-architecture §9）。**Blocked on upstream gating**：DbService `createSnapshot` / `applyMigrations` / **preboot promotion gate** + 各模块 write-quiesce 接口——见 `/Users/gd32/Downloads/backup-upstream-issues/`。 |
