...
import type { FileResourceContext } from '@main/data/db/backup/contexts'
...
async function collectNotesMarkdown(ctx: FileResourceContext): Promise<Set<string>> {
  ...
  try {
    ...
    if (code === 'ENOENT') {
      ctx.logger.warn('PREFERENCES collectFileResources: notes root does not exist (no notes configured)', {
        notesRoot: root
      })
      return out
    }
  ...
  while (stack.length > 0) {
    ...
    try {
      ...
      ctx.logger.warn('PREFERENCES collectFileResources: unreadable subtree skipped', {
        dir,
  ...
  ctx.logger.info('PREFERENCES collectFileResources: collected notes markdown files', {
    count: out.size
  })
  return out
}
