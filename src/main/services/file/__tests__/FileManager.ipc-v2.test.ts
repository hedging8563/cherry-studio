...
describe('FileManager v2 IPC handler registration', () => {
  ...
  it('registers File:setCleanupPolicy IPC channel', () => {
    const registeredChannels = vi.mocked(ipcMain.handle).mock.calls.map(([channel]) => channel)
    expect(registeredChannels).toContain(IpcChannel.File_SetCleanupPolicy)
  })

  it('setCleanupPolicy handler flips the policy in both directions', async () => {
    const createHandler = vi
      .mocked(ipcMain.handle)
      .mock.calls.find(([ch]) => ch === IpcChannel.File_CreateInternalEntry)?.[1]
    const entry = await createHandler!({} as never, {
    ...
    const setPolicyHandler = vi
      .mocked(ipcMain.handle)
      .mock.calls.find(([ch]) => ch === IpcChannel.File_SetCleanupPolicy)?.[1]
    expect(setPolicyHandler).toBeDefined()

    const toAuto = await setPolicyHandler!({} as never, { id: entry.id, cleanupPolicy: 'delete_when_unreferenced' })
    expect(toAuto.cleanupPolicy).toBe('delete_when_unreferenced')

    const toManual = await setPolicyHandler!({} as never, { id: entry.id, cleanupPolicy: 'manual' })
    expect(toManual.cleanupPolicy).toBe('manual')
  })

  it('setCleanupPolicy rejects an invalid policy value at the schema boundary', async () => {
    const createHandler = vi
      .mocked(ipcMain.handle)
      .mock.calls.find(([ch]) => ch === IpcChannel.File_CreateInternalEntry)?.[1]
    const entry = await createHandler!({} as never, {
    ...
    const setPolicyHandler = vi
      .mocked(ipcMain.handle)
      .mock.calls.find(([ch]) => ch === IpcChannel.File_SetCleanupPolicy)?.[1]
    await expect(setPolicyHandler!({} as never, { id: entry.id, cleanupPolicy: 'not-a-policy' })).rejects.toThrow()
  })
})
