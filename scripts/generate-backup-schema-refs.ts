...
interface TableInfo {
  ...
  /** Columns declared with Drizzle text({ mode: 'json' }) — runtime marker dataType === 'json'. */
...
function extractTableInfo(table: SQLiteTable, autoIncrementKeys: Set<string>): TableInfo {
  ...
  // JSON columns: Drizzle text({ mode: 'json' }) sets the hidden runtime marker
  // column.dataType === 'json' (same hidden-prop cast idiom as .primary/.notNull/
  // .isUnique/.autoIncrement above). getSQLType() still returns 'text', so this is
  // the only way to distinguish a JSON-serialized column from a plain text column.
  const jsonColumns = config.columns
    .filter((column) => (column as { dataType?: string }).dataType === 'json')
    .map((column) => keyLookup.get(column) ?? column.name)

  return { name: config.name, columns, primaryKey, foreignKeys, uniqueKeys, jsonColumns }
}
...
function emitDbSchemaRefs(
  ...
  // JSON columns per table (empty array when none). Emits an entry for EVERY table so
  // the Readonly<Record<DbTableName, …>> shape holds without a partial-keyed record.
  const jsonColumnsByTable = sortedTables
    .map((t) => {
      if (t.jsonColumns.length === 0) return `  ${t.name}: []`
      const cols = t.jsonColumns.map((c) => `    ${str(c)}`).join(',\n')
      return `  ${t.name}: [\n${cols}\n  ]`
    })
    .join(',\n')
